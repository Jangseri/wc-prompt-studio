...
// model is forced to emit the exact region-composition shape the
// regions-step UI expects. Any drift in the type MUST be reflected here.
...
- description: 업무/회사 소개 2-3문장
  - 업종이 "병원" 또는 "일반"이면 서버가 [입력 정보] 블록([입력 정보], AA2001/AA2002/AA2003 포함)을 description 앞에 자동 삽입합니다. 따라서 description에는 이 블록을 직접 포함하지 말고 업무 소개만 작성하세요.
- greeting: 소스에 명시된 시스템 초기 멘트가 있으면 원문 그대로, 없으면 빈 문자열

### answerScope
- rag.enabled: QnA/지식베이스 기반 응답이 필요하면 true
- rag.performanceNotes: **사용하지 않습니다. 항상 빈 문자열("")로 두세요.**
- specifics.type: 기본값 "keyValue"로 두세요. (사용자가 UI에서 직접 선택합니다.)
- specifics.keyValueItems: **사용자가 직접 입력합니다. 빈 배열([])로 두세요.**
- specifics.sentence: **사용자가 직접 입력합니다. 빈 문자열("")로 두세요.**

### branching
- **이 영역은 대화 흐름 전체를 정의하는 가장 중요한 섹션입니다.** 소스 데이터의 흐름을 구조화하여 채우세요.
- topLevelRules: 대화 흐름 시작 전 반드시 지켜야 하는 절대 규칙/금지사항 리스트.
  - 소스 데이터에 근거가 있을 때만 작성. 없으면 빈 배열 [].
  - 예: ["신환 여부 답변 전에는 업무 시간 안내를 절대 하지 않는다", "이를 위반하면 오답이다"]
- steps: 대화 흐름을 **순서대로** 나열한 단계 배열.

#### A. steps 분할 기준
- 소스 데이터의 분기 노드 하나당 하나의 step 을 만듭니다. 여러 분기를 한 step 에 뭉치지 마세요.
- 엑셀 소스의 \`[colN]\` 마커는 트리 깊이를 의미합니다. 같은 N = 형제 단계, N 증가 = 자식 단계 → step 분할 기준으로 활용하세요.
- 권장 골격 (소스에 해당 흐름이 있을 때만 적용):
  1. **분류** — 예: "부서 확인", "문의 유형 확인"
  2. **정보 수집** — 예: "신환 여부 확인", "예약 정보 수집"
  3. **액션 처리** — 예: "데스크 연결", "티켓 생성", "콜백 등록"
  4. **종료/예외** — 예: "통화 종료", "재연락 안내"
- 같은 멘트가 단순 반복되는 경우는 한 step 으로 합치되, 분기마다 다른 액션이 일어나면 별도 step 으로 분리합니다.

#### B. 각 step 의 필드
- id: 순서를 반영한 고유 문자열. "step-1", "step-2", "step-3" 형식으로 부여.
- title: 명확한 동사구 (예: "부서 확인", "신환 여부 질문", "예약 정보 수집").
- body: 아래 sub-섹션을 **고정 순서로** 작성합니다. 해당 항목이 없는 step 은 그 sub-섹션을 통째로 생략하고 나머지만 채웁니다.
  - **설명**: 이 단계의 목적/상황 한 줄.
  - **수집 항목**: 접수·예약일 때만. 한 줄에 하나씩 (예: \`- 성함\`, \`- 연락처\`).
  - **멘트**: 소스 원문 그대로 큰따옴표로 감싸 (단어·조사 변경 금지). 여러 줄이면 줄마다 큰따옴표.
  - **분기**: 조건 분기가 있을 때만. IF/ELSE 블록으로 들여쓰기:
    \`\`\`
    IF <부서 = "closed">:
        → 재연락 안내
    ...
    ELSE:
        → 통화 연결
        "확인 감사합니다. OOO로 연결해드리겠습니다..."
        다음 단계: step-M
    \`\`\`
  - **다음 단계**: 순차 흐름일 때만 (분기가 있으면 분기 안에 명시). \`step-N\` 형식.
  - **주의**: 보조 사항이 있을 때 "※" 기호로.

- 본문 markdown 안에서:
  - 액션은 "→" 로 시작합니다. 예: "→ 고객의 문의 내용에 따라 부서를 식별합니다."
  - 고객에게 전달할 멘트는 반드시 큰따옴표로 감싸 소스 원문 그대로 복사합니다. 예: "저희 병원에 방문하신 적이 있나요?"

### toolCalling
- **이 영역은 현재 비활성 상태입니다. mcp / api / agent / dataQuery 네 필드 모두 빈 문자열("")로 두세요.** UI에서도 편집이 막혀 있으며, 최종 출력에도 포함되지 않습니다.

### system
- rules: **서버가 채널별 고정 기본값(콜봇=STT/TTS 규칙, 챗봇=Chat 응답 원칙·스타일 가이드)으로 덮어씁니다. 빈 문자열("")로 두세요.**
...
- customNotes: **사용하지 않습니다. 항상 빈 문자열("")로 두세요.** (별도 "커스텀 섹션" 영역이 대신 사용됩니다.)

### custom
- items: **사용자가 직접 추가하는 섹션입니다. 빈 배열([])로 두세요.**`;

const REGIONS_SYSTEM_CALLBOT_SUFFIX = `

## 채널: 콜봇
- 음성 기반. 한 문장 간결 응답을 원칙으로 persona.tone에 반영합니다.
- greeting이 있으면 참고용 원문 그대로 보존합니다(시스템 송출 멘트 포함).`;

const REGIONS_SYSTEM_CHATBOT_SUFFIX = `

## 채널: 챗봇
- 타이핑 입력. JSON Object 형태(messages + reactionType)로 실제 응답이 송출됨을 고려해 persona.tone에 명시합니다.
- **각 step.body 의 마지막에 \`- **reactionType**: <값>\` 한 줄을 반드시 추가합니다.** 값은 소스에 정의된 것을 우선 사용하고, 없으면 dialog / reaction_saveReception / prompt_analysis_summary 중 흐름에 맞는 것을 고릅니다.
- 접수 플로우는 진행 중과 완료 시 reactionType 이 다릅니다. 진행 중에는 "dialog" 유지, 접수 완료 시점에 reaction_saveReception 또는 prompt_analysis_summary 로 전이 — 이 전이를 step.body 의 reactionType 줄에 그대로 명시합니다 (예: "진행 중 dialog → 완료 시 reaction_saveReception").
...
export function buildRegionsUserPrompt(input: {
  ...
  let prompt = `다음 소스 데이터를 바탕으로 ${input.channel === "chatbot" ? "AI 챗봇" : "AI 콜봇"} 프롬프트를 JSON Schema가 정의한 구성 구조에 맞춰 작성하세요.
