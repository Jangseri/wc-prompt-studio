...
- steps: 대화 흐름을 **순서대로** 나열한 단계 배열. 각 step은 아래 3개 필드를 모두 채웁니다.
  - id: 순서를 반영한 고유 문자열. "step-1", "step-2", "step-3" 형식으로 부여.
  - title: 단계 이름 (예: "부서 확인", "신환 여부 확인", "업무 시간 확인")
  - body: 해당 단계의 action·설명·멘트·분기 로직을 담은 자유 markdown 텍스트.
    - 각 action은 "→" 로 시작합니다. 예: "→ 고객의 문의 내용에 따라 부서를 식별합니다."
    - 고객에게 전달할 멘트는 반드시 쌍따옴표로 감싸 원문 그대로. 예: "저희 병원에 방문하신 적이 있나요?"
    - 조건 분기가 있으면 IF/ELSE 블록으로 들여쓰기하여 명시:
      \`\`\`
      IF <부서 = "closed">:
          → 재연락 안내
...
- 접수 플로우가 있으면 해당 step.body에 reactionType 전이까지 포함합니다.
